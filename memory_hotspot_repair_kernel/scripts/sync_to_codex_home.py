...
def _copy_skill(source: Path, destination: Path) -> None:
    ...
    shutil.copytree(source, destination, ignore=shutil.ignore_patterns("runs", "__pycache__", "*.pyc"))


def main() -> int:
    parser = argparse.ArgumentParser(description="Sync repo-local skills into the Codex skills directory.")
