...
For this skill bundle, the recommended primary entry point is:

- `$memory-optimizer-agent`

Use the underlying skills directly only when you explicitly want one isolated step such as:

- hotspot inspection only
- remediation only
- post-fix review only
...
- `python memory-optimizer-agent/scripts/orchestrate.py --config <run.yaml>`
- `python memory-optimizer-agent/scripts/orchestrate.py --config <run.yaml> --profile-name <profile>`
- `python memory-optimizer-agent/scripts/orchestrate.py --config <run.yaml> --preflight-only`
- `python memory-optimizer-agent/scripts/orchestrate.py --list-runs`
- `python memory-optimizer-agent/scripts/orchestrate.py --dashboard`
- `python memory-optimizer-agent/scripts/orchestrate.py --resume-latest-failed`
- `python memory-optimizer-agent/scripts/orchestrate.py --resume-run <run-name> --partial-rerun-from-round 1`
- `python scripts/sync_to_codex_home.py --dry-run`
- `python scripts/sync_to_codex_home.py`
...
- Use `python scripts/sync_to_codex_home.py` to copy the repo-local skills into `$CODEX_HOME/skills` or `~/.codex/skills`.
...
- The portable package does not auto-bundle repo-specific overlays into the default core workflow.
- In this multi-skill hub, repo-specific overlays and example run configurations are stored under the repository `examples/` tree.
